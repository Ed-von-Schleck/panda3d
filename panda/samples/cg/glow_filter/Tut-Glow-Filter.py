...
class GlowDemo(DirectObject):
  ...
  # Post the instructions
        self.title = addTitle("Panda3D: Tutorial - Using Cg in Panda (Glow Filter)")
        ...
        # load our model
        self.tron=Actor.Actor()
        self.tron.loadModel(MYDIR+"/Tron/tron")
        self.tron.loadAnims({"running":MYDIR+"/Tron/tron_anim"})
        self.tron.reparentTo(render)
        self.interval = self.tron.hprInterval(60,Point3(360,0,0))
#        self.interval = self.tron.hprInterval(60,Point3(0,0,0))
        self.interval.loop()
        ...
        dlight.setColor(VBase4(1.0, 0.7, 0.2, 1))
        ...
        glowAttrib = ShaderAttrib.make()
        glowAttrib = glowAttrib.setShader(glowShader)
        glowCamera.node().setInitialState(render.getState().addAttrib(glowAttrib))
